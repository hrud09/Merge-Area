...
public class GameplayPanelControl : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (!useLevelNumber) levelNumberText.transform.parent.gameObject.SetActive(false);
    ...
    private void OnEnable()
    {
        if (useLevelNumber)
        {
            levelNumberText.text = "level " + PlayerPrefs.GetInt(Constants.CurrentLevel).ToString();
        }
        ...
        {
            Constants.FadeInOutAnimation(tapToPlayText, .5f, .35f);
        }
